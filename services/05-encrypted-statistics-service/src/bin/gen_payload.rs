...
use std::fs;
use tfhe::prelude::*;
use tfhe::{ClientKey, CompressedServerKey, ConfigBuilder, FheInt16, FheInt32, FheInt8};
...
fn main() {
    ...
    let list = encrypt_list_i16(
        &[1000, 4200, 700, 9900, 2300, 5500, 300, 7700, 3100, 6000],
        &client_key,
    );
    fs::write(
        "payload_list_n10_b16.txt",
        serde_json::to_string(&list).unwrap(),
    )
    .unwrap();
    ...
    let list = encrypt_list_i32(
        &[
            100_000, 420_000, 70_000, 990_000, 230_000, 550_000, 30_000, 770_000, 310_000, 600_000,
        ],
        &client_key,
    );
    fs::write(
