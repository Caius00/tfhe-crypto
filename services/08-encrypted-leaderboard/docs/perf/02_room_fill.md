...
- **Konfig:** `MAX_PLAYERS=20`, `STAGE_DURATION_SEC=60` (10 Tempo-Stufen pro Runde von 10 s → 1 s), `PAUSE_DURATION_SEC=120` (zwischen Runden)
...
- Die Spitzen entstehen **systematisch zu Beginn jeder Runde**: nach der 2-min-Pause submitten alle aktiven Spieler quasi gleichzeitig wieder ihren ersten Wert → FHE-Queue + Background-Sort stauen sich kurz. Nach ein paar Sekunden desynchronisieren sich die Spieler und die Latenz normalisiert sich wieder.
- **Kein systemischer Crash** — Service bewältigt 20 parallele Spieler bei 1-s-Tempo, p95 bleibt insgesamt unter 3 s.
