...
\begin{itemize}
    \item \textbf{Verbindlichen Vorschriften}, die für die Erstellung von wissenschaftlichen Arbeiten unbedingt einzuhalten sind und
...
\hline
     Praxisprojekt (Bachelor) & 20 bis 22 Seiten \\\hline
     Bachelor Thesis & 40 bis 44 Seiten \\\hline
     Praxisprojekt (Master) & 30 bis 33 Seiten \\\hline
     Master Thesis & 60 bis 66 Seiten \\\hline
     Transferprojekt (Praxisprojekt Aufbaustudium) & 20 bis 25 Seiten \\\hline
\end{tabular}
...
\begin{enumerate}
    \item Titelblatt mit Namen, Nachnamen, Studiengang, Namen des Prüfers (Praxisarbeit) oder der Prüfer (Erst- und Zweitprüfer bei Bachelor- und Master-Thesis), sowie Abgabedatum
    ...
    \item  \textit{Optional:} Ein Glossar
...
\begin{itemize}
    ...
    \item Wenn Sie neue Begriffe definieren oder erklären, verwenden Sie dabei den \textit{Kursivdruck}. Wichtige Schlagworte im Text können \textbf{fett} hervorgehoben werden. Aber: setzen Sie dieses Stilmittel nur sparsam ein! Erläutern Sie solche Konventionen im Abschnitt ''Struktur der Arbeit, Methodik'', siehe \ref{sec:struktur-projektbericht}.
\end{itemize}
...
Der Inhalt der Arbeit muss in Kapitel und Abschnitte strukturiert sein. Abschnitte können Unterabschnitte enthalten. Hat ein Abschnitt einen Unterabschnitt, sollte er auch (mindestens) einen zweiten haben. Nummerieren Sie Kapitel, Abschnitte und Unterabschnitte nach dem Dezimalsystem (1, 1.1, 1.1.1 etc.). 
...
Achten Sie darauf, dass jede Abbildung gut lesbar ist und nicht den Rand des Textbereichs überschreitet. Eine Abbildung muss immer im Text referenziert und beschrieben werden. Was sieht der Leser? Was bedeuten zum Beispiel verschiedene Pfeile? Eine Legende in der Grafik (wie in Abb.\,\ref{fig:feature-diagram-productioncell-example}) ist nicht zwingend notwendig, kann die Grafik aber besser verständlich machen. Sie können das Wort "`Abbildung"' im Text mit "`Abb."' abkürzen. An Satzanfängen sieht die ausgeschriebene Variante allerdings schöner aus.
...
Schreiben Sie besser \enquote{Ich habe eine Problemanalyse durchgeführt und Anforderungen abgeleitet}. Haben vielleicht Kollegen dabei geholfen? Welchen Anteil hatte dabei die Kollegin? Machen Sie dies explizit deutlich und schreiben Sie zum Beispiel: \enquote{Ich habe zusammen mit Mitarbeitern eine Problemanalyse durchgeführt und eine erste Liste von Anforderungen abgeleitet.} Nun wird deutlich, welchen Beitrag Sie geleistet haben! Ihre Prüfer können Ihre Leistung nun eindeutiger beurteilen.

Gehen Sie jedoch sparsam mit \enquote{ich} oder \enquote{wir} um und beschränken Sie deren Verwendung auf Stellen, an denen Sie Ihr eigenes Vorgehen, Abwägungen oder Entscheidungen beschreiben -- {\em eine Praxis- oder Abschlussarbeit ist kein Tagebuch und keine Erzählung aus der Ich-Perspektive!}
...
Bei der Erklärung von Grundlagen, Problemstellungen, Lösungsansätzen, verwandten Arbeiten oder zur Unterstützung von Argumenten sollten Sie auf geeignete Quellen hinweisen. Hier Beispiele für Referenzen auf Quellen~\cite{Greenyer2007,DBLP:conf/calco/KonigWK23}, welche im Quellverzeichnis aufgelistet sind. 
...
\begin{itemize}
    ...
    Darauf folgt optional Serie, Heftnummer und Band, und schließlich Seitenbereich und Jahresangabe. Beispiel: \cite{DBLP:conf/calco/KonigWK23}
...
\begin{itemize}
  ...
  \item \url{https://www.springerprofessional.de/} (die FHDW hat hier Zugänge -- sprechen Sie Ihren Betreuer darauf an.)
...
In Praxisarbeiten kann der Großteil der Quellverweise aus Internetquellen kommen, weil es sich dort oft um eine sehr praxisnahe Fragestellung handelt. Bitte überprüfen Sie solche Seiten aber auf Ernsthaftigkeit und Tiefgang. Chats sind keine guten Quellen, Wikipedia hat sich mittlerweise als weitgehend zuverlässig erwiesen. Bei einer Bachelor- oder Master-Thesis sollten jedoch auch, wenn nicht gar hauptsächlich, wissenschaftliche Quellen genannt werden (siehe auch Kapitel~\ref{chap:struktur-und-inhalt}).
...
\section{Nutzung generativer KI oder sonstiger KI-unterstützter Werkzeuge}
KI-basierte text- oder bildgenerierende Werkzeuge können bei der Erstellung wissenschaftlicher Arbeiten verwendet werden. Übernehmen Sie aber niemals unreflektiert die Ausgabe solcher Algorithmen! Überprüfen und überarbeiten Sie die generierten Texte und fragen Sie sich, ob Sie die überarbeiten Passagen vollständig verstanden haben. Weitere Hinweise zu diesem Thema finden Sie \hyperlink{https://research-and-innovation.ec.europa.eu/news/all-research-and-innovation-news/guidelines-responsible-use-generative-ai-research-developed-european-research-area-forum-2024-03-20_en}{hier}.
\par 
{\em Wenn nachgewiesen wird, dass generierte Inhalte unbearbeitet übernommen wurden, wird dies als Täuschungsversuch gewertet.}